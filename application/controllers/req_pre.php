...
class req_pre extends CI_Controller{
	...
	{		
		...
		$this->session->set_userdata('action_img', 'Req_predef');
	}
	
	function acceuil(){
		$this->load->model("get_db");
		$data["menu_pre"] = $this->get_db->get_liste("SELECT code, description FROM arbre WHERE type = 'titre' and pere like '".$_GET["bdd"]."%' ORDER BY code");
		...
		//echo "SELECT id, code, description, pere FROM arbre WHERE type = 'predefinie' and pere like '".$_GET["bdd"]."%' ORDER BY code";
