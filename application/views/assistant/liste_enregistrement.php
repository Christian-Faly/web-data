<div id="list_req" style="width:20%;float:left">
	<div class="tableaux">

		<table>
			<!--<thead>
				<tr>
					<th class='accordth'><h2 style="background-color:#bdb76b";>ASSISTANT REQUETE</h2></th>
				</tr>
			</thead>
-->
			<tbody>
							...
							foreach($menu_enr as $row){
								if ((strlen($row->code)>5) and  (strlen($row->code)<9)){
									if  (strlen($row->code)<7){
										...
										echo "<h4 style='margin:0px 0px 20px 0px; border: 1px #556b2f solid; background-color:rgb(195, 224, 121); box-shadow: 6px 6px 0px #808000;'>".$row->description."</h4>";
										echo "<div>";
									}else{
										echo "<p id = '".$row->code."' alt='".$row->nom_table."' style='border: 1px #ff7f50 solid; margin:0px 0px 5px 10px; box-shadow: 3px 3px 0px #ff7f50'>".$row->description."</p>";
									}
								}
		...
		<tbody id="tb">
			<tr class="titleTr">
        		...
        		<td style="text-align:right;font-family:Calibri;">
					...
					<span id="b_rechA" style="font-size:13px;font-family:Calibri;line-height:12px;float:right;margin-right:20px;margin-left:5px" class="btn btn-primary btnxs"><i class="icon icon-search icon-white"></i></span>
...
<div id="ancre" style="width:79%;float:right;text-align:center">
			<button id="vista" style="font-size:12px;margin-top:5px;width:250px;" name="action" value="sql_liste" type="submit"> <i class="icon icon-play icon-white"></i>  Visualiser Tableau</button>
            <button id="b_a_r" style="font-size:12px;margin-top:5px;width:250px;"type="button"><i class="icon icon-columns icon-white"></i> Assistant requete</button>
            <span id="s_cont">
	...
	<div id="assist_req_corps">
		...
		if(isset($nom_table)){
            ...
            $data = array(
                ...
                'type' => 'submit',
                'content' => '<i class="icon icon-code icon-white"></i> Visualiser SQL'
            );
            ...
            $data = array(
                ...
                'value' => 'critere_liste',
                'type' => 'submit',
                'content' => '<i class="icon icon-play icon-white"></i> visualiser tableau',
				'target' => "_blank"
            );
            echo form_button($data);
            ...
            $data = array(
                ...
                'type' => 'submit',
                'content' => '<i class="icon icon-plus icon-white"></i> oRequete croise'
            );
            echo form_button($data);
