<div id="div_tb_ainserer">
	<?php
		...
		//echo $sql." limit ".($nbparpage +1 )." OFFSET ".$offset;
		if (!$resultat) {
		...
		echo '<table id="visu_tb_par_region" class="table table-hover" border="1" width="100%">'."\n";//style="table-layout:fixed;" bgcolor="#FFFFFF"
		//echo '<table style="table-layout:fixed;" bgcolor="#FFFFFF" border="1">'."\n";//
				...
				echo '<tr bgcolor="#FFCC00">';
					...
					echo '<th hidden scope="col">SQL</th>';
				...
				elseif ($row['niveau']==5){
					...
					if ($row["sql_web"]>""){
						...
						foreach($re->result_array() as $ligne){
							$t_a=$ligne["epra"]; $t_b=$ligne["eprb"];$t_g=$ligne["eprg"];$t_h=$ligne["eprh"];$t_i=$ligne["epri"];  
							$t_l=$ligne["eprl"]; $t_s=$ligne["eprs"];$t_t=$ligne["eprt"];$t_v=$ligne["eprv"];     
						} 
						...
						$t_a=NULL;$t_b=NULL; $t_g=NULL; $t_h=NULL; $t_i=NULL;	$t_l=NULL; $t_s=NULL; $t_t=NULL; $t_v=NULL;     
					}
					...
					$pt1=NULL; $pt2=NULL;$pt3=NULL;	$pt4=NULL;$pa=NULL;
					$re2 = $this->db->query($sql2);
					foreach($re2->result_array() as $ligne){
						$pt1=$ligne["pt1"]; $pt2=$ligne["pt2"];	$pt3=$ligne["pt3"];	$pt4=$ligne["pt4"];	$pa=$ligne["pa"];   
					}
					echo '<td>';
					echo $row['id'];
					...
					echo '<td>'.$row["unite"].'</td>';
						...
						*/
						...
						echo '<td id="td_'.$row["id"].'_a" alt="4"><a  href="#td_'.$row["id"].'_a">'.$t_a.'</a></td>';
						echo '<td id="td_'.$row["id"].'_b" alt="8"><a  href="#td_'.$row["id"].'_b">'.$t_b.'</a></td>';
						echo '<td id="td_'.$row["id"].'_g" alt="6"><a  href="#td_'.$row["id"].'_g">'.$t_g.'</a></td>';
						echo '<td id="td_'.$row["id"].'_h" alt="15"><a href="#td_'.$row["id"].'_h">'.$t_h.'</a></td>';
						echo '<td id="td_'.$row["id"].'_i" alt="3"><a  href="#td_'.$row["id"].'_i">'.$t_i.'</a></td>';
						echo '<td id="td_'.$row["id"].'_l" alt="13"><a href="#td_'.$row["id"].'_l">'.$t_l.'</a></td>';
						echo '<td id="td_'.$row["id"].'_s" alt="7"><a  href="#td_'.$row["id"].'_s">'.$t_s.'</a></td>';
						echo '<td id="td_'.$row["id"].'_t" alt="12"><a href="#td_'.$row["id"].'_t">'.$t_t.'</a></td>';
						echo '<td id="td_'.$row["id"].'_v" alt="16"><a href="#td_'.$row["id"].'_v">'.$t_v.'</a></td>';
					echo '</tr>';
				}
			}
